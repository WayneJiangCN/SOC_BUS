...
inline bool
is_supported_case(const std::string& name)
{
    return name == "single_rw" || name == "multi_master" ||
           name == "multi_target_linear" || name == "backpressure";
}
...
print_usage(const char* program)
{
    std::cout
        << "Usage: " << program
        << " [pem_config_cloud.toml] [case] [options]\n\n"
        << "Cases: single_rw, multi_master, multi_target_linear, "
           "backpressure\n\n"
        << "Topology/workload:\n"
        << "  --masters N                 number of independent cores\n"
        << "  --targets N                 number of memory targets\n"
        << "  --uops N                    reads generated by each core\n"
        << "  --cycles N                  simulation cycle limit\n"
        << "  --interleave none|linear|xor target address mapping\n"
        << "  --interleave-size N         interleave granularity in bytes\n\n"
        << "  --interleave-hash-shift N\n"
        << "  --interleave-hash-seed N\n\n"
        << "Latency and bottlenecks:\n"
        << "  --target-frontend-latency N\n"
        << "  --target-forward-latency N\n"
        << "  --target-response-latency N\n"
        << "  --target-header-latency N\n"
        << "  --target-width N            target interface bytes/cycle\n"
        << "  --hotspot-threshold N       OSD threshold for hotspot penalty\n"
        << "  --hotspot-penalty N         extra busy cycles past threshold\n"
        << "  --link-latency N            ring link latency in cycles\n"
        << "  --link-width N              ring link bytes/cycle\n"
        << "  --link-max-inflight N\n"
        << "  --master-fifo-depth N\n"
        << "  --target-fifo-depth N\n"
        << "  --ring-fifo-depth N\n\n"
        ...
        << "  --master-wr-osd N\n"
        << "  --global-osd N\n"
        << "  --target-rd-osd N           0 inherits memory config\n"
        << "  --target-wr-osd N           0 inherits memory config\n"
        << "  --target-acc-osd N          0 inherits memory config\n\n"
        << "Performance reporting:\n"
        << "  --perf-target PCT           utilization target (default 80)\n"
        << "  --clock-ghz FREQ            convert B/cycle to GB/s\n"
        << "  --require-perf-target       fail when target is not reached\n";
}
...
inline bool
split_option(int argc, char** argv, int* index, std::string* name,
...
inline bool
parse_options(int argc, char** argv, RingDemoConfig* tc, std::string* error)
{
    for (int i = 3; i < argc; ++i) {
        const std::string arg = argv[i];
        if (arg == "--require-perf-target") {
        ...
        std::string name;
        std::string value;
        if (!split_option(argc, argv, &i, &name, &value, error) ||
            !apply_option(name, value, tc, error)) {
            return false;
        }
    }
    return true;
...
run(int argc, char** argv)
{
    ...
    if (!is_supported_case(case_name)) {
        std::cerr << "Unknown case: " << case_name << std::endl;
        print_usage(argv[0]);
        return 2;
    }

    RingDemoConfig test_case = make_demo_case(case_name);
    std::string error;
    if (!parse_options(argc, argv, &test_case, &error)) {
        if (error == "help") {
            print_usage(argv[0]);
            return 0;
        }
    ...
    if (!validate_config(test_case, &error)) {
