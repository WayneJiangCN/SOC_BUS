...
默认路径已经固定为 `../etc/tm_ring_demo.toml` 和
`../etc/pem_config_cloud.toml`，均相对于测试程序的运行目录；正常跑默认多核
GTest 时不需要再设置环境变量。Ring 场景 TOML 仍通过 `ddr_config` 字段记录
DDR TOML 路径。命令行 `--ddr-config` 或脚本参数 `-DdrConfig` 可以覆盖该路径；旧名称
...
```powershell
.\run_tm_ring_esl.ps1 -Binary C:\path\to\tm_ring_esl.exe `
  -Set ring_link_width_bytes=8,ring_link_latency=2
```
